function donutChart(config) {
    ...
    var update, 
        ...
        colour = this.d3.scaleOrdinal(config.colors), // colour scheme
        variable = config.variable || "sumTransactionsEur", // value in data that will dictate proportions on chart
        category = config.category || "type", // compare data by
        padAngle = config.padAngle ||  0.15, // effectively dictates the gap between slices
        transTime = config.transTime || 750, // transition time
        updateData,
        floatFormat = this.d3.format('.4r'),
        cornerRadius = config.cornerRadius || 3, // sets how rounded the corners are on each slice
        percentFormat = this.d3.format(',.2%')
        formatter = config.formatter, 
        translate = config.translate,
        handleTypeClicked = config.handleTypeClicked;

    if (data.length == 0){
        data.push(createDefaultItem());
    }

    function createDefaultItem(){
        let itm = {};

        itm[config.variable] = 0.0000001;
        itm[config.category] = "";
            ...
            */
                ...
                selection.on('mouseenter', function (d) {
                    ...
                    if (data[0][category] != ""){
                        ...
                        svg.append('text')
                            ...
                            .html(toolTipHTML(d, false)) // add text to the circle.
            ...
            function defaultToolTip(){
                ...
                svg.append('text')
                        .attr('class', 'toolCircle')
                        .attr('dy', -15) // hard-coded. can adjust this to adjust text vertical alignment in tooltip
                        .html(toolTipHTML(data, true)) // add text to the circle.
                        .style('text-anchor', 'middle'); // centres text in tooltip
            ...
            function toolTipHTML(inputData, isAll) {
                ...
                if (isAll){
                    ...
                    for (var i=0; i<inputData.length;i++){
                        sum += parseFloat(inputData[i][variable])
                    }

                    if (data[0][category] == ""){
                        tip += '<tspan class="title" x="0" >' + "Keine Ausgaben" + '</tspan>'
                    }else{
                        tip += '<tspan class="title" x="0" >' + translate.instant("DONUT_TOTAL") + '</tspan>'
                    }
                    tip += '<tspan class="amount" x="0" dy="1.2em">' + formatter.formatMoney(sum) + '</tspan>'
                }else{
                    
                    tip += '<tspan class="title" x="0" >' + inputData.data[category] + '</tspan>'
                    tip += '<tspan class="amount" x="0" dy="1.2em">' + formatter.formatMoney(inputData.data[variable]) + '</tspan>'
                }
