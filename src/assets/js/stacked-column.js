function createColumnChart (selection, config, inputData) {

    var margin = {top: 20, right: 20, bottom: 30, left: 40},
        width = config.width - margin.left - margin.right  || 500,
        height = config.height - margin.top - margin.bottom || 500,
        formatter = config.formatter,
        variable = config.variable || "name", 
        measure = config.measure, 
        maxBarWidth = config.maxBarWidth || 90 ;

    var parsedDataObj = parseData(inputData);
    var data = parsedDataObj.data; 
    let valueKeys = parsedDataObj.valueKeys;

    const colors = config.colors || ['#98abc5', '#8a89a6', '#7b6888', '#6b486b', '#a05d56', '#d0743c', '#ff8c00'];
    ...
    let nameKeys = data[Object.keys(data)[0]].map(obj =>obj["key"])
    ...
    function parseData(inputData){
        ...
        nestedData.forEach(element => {
            ...
            var nestedDataTime = d3.nest()
                .key(function(d) { return d.time; }) 
                .entries(element.values);
    
            nestedDataTime.forEach(elem => {
                ...
                elem.values.forEach(e => {
                    ...
                    itm.values[e[variable]] = parseFloat(e[measure]);
    
                    if (valueKeys.indexOf(e[variable]) == -1){
                        valueKeys.push(e[variable])
                    }
